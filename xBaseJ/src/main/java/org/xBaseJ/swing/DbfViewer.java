...
public class DbfViewer extends JFrame
		implements ActionListener, WindowListener, ListSelectionListener, DocumentListener, Printable

{
	...
	private boolean fileChangedTestDontContinue() {
		...
		return JOptionPane.showConfirmDialog(null, "Data has changed. Ok to continue. Cancel to return.", "Confirm",
				dialogButton) == JOptionPane.CANCEL_OPTION;
	...
	*/
	...
	public void save() {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(parent, inString + " caused an exception: " + e.getMessage(),
	...
	public void setValueAt(Object in, int r, int c) {
		 ...
		 */
			...
			JOptionPane.showMessageDialog(parent,
					"row: " + r + " col: " + c + " " + in + " caused an exception: " + e.getMessage(),
					"Error Setting Field", JOptionPane.ERROR_MESSAGE);
		}
	}
...
class dbfTableRecordMemoDialog extends JDialog implements ActionListener, WindowListener {
	...
	public dbfTableRecordMemoDialog(DbfViewer inParent, JPanel inFr, dbfTableModel indbfTableModel, int inRow,
			int inCol) {
...
class dbfViewerSearchDialog extends JDialog implements ActionListener, WindowListener {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == Okay) {
			...
			if (s.length() < 1) {
				JOptionPane.showMessageDialog(parent, "Enter Search Text", "Enter Search Text",
			...
			if (i == model.getColumnCount() - 1) {
				JOptionPane.showMessageDialog(parent, "Select one or more fields to search in", "Select A Field",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
