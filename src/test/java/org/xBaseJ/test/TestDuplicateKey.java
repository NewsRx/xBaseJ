/**
 * is there a known bug regarding find_rec(NodeKey, Node, int) in NDX.java while updating a record
 * with non-unique index? In my oppinion this line: for (aNode.set_pos(0); aNode.get_pos() < until
 * && stat > 0; aNode.pos_up()) (see full method attached) is handling "stat" not appropiately. When
 * stat is 0, but the record found is not the one we are searching for, there is no loop! We will
 * find the record we are looking for if this line goes like: for (aNode.set_pos(0); aNode.get_pos()
 * < until && stat >= 0; aNode.pos_up()) Result: A new entry is being added even if there IS a
 * matching record (=> duplicate entries)!
 */
