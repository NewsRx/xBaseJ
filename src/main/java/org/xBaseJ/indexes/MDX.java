...
public class MDX extends Index {
  ...
  private int update_entry(MNode aNode, NodeKey NDXkey, int recno, int oldrecno)
    ...
    if (aNode == null) {
      ...
      MNode newNode =
          new MNode(
              mfile, key_per_Node, key_length, keyType, mfile.anchor.get_nextavailable(), true);
  ...
  private void splitNode(MNode aNode, int savepos) throws xBaseJException, IOException {
    ...
    if (savepos > i) {
      ...
      aNode.set_record_number(mfile.anchor.get_nextavailable()); // did a
      // split
      // renumber
      // Node
      mfile.anchor.update_nextavailable();
