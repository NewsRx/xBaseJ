 ...
 *     <p>Change History Date Developer Desc 20091007 Roland Hughes (rth) key size was wrong for
 *     date and numeric changed from 16 to 8
 *     <p>20091010 Roland Hughes (rth) reIndex() was using read() and should have been using
 *     gotoRecord(). read() is influenced by any attached NDX.
 *     <p>20110420 Joe McVerry (jm) removed stat > 0 from from for loop in find_entry. this was
 *     creating duplicate index update problem.
 */
import java.io.File;
...
public class NDX extends Index {
   ...
   * @throws IOException exception thrown by calling methods
   * @throws xBaseJException most likely cause - file not found
   */
  ...
  private void splitNode(Node aNode, int savepos) throws xBaseJException, IOException {
    ...
    if (savepos > i) {
      ...
      if (aNode.get_prev() != null) {
        ...
        bNode.set_pos(j - 1);
        update_entry(
            bNode, // pass it aNode for it 's creating a new
            // top Node
            bNode.get_key_value(),
            left,
            0,
  ...
  public void del_entry(Node inNode) throws IOException, xBaseJException {
    ...
    } /* endif */ else if (pos < aNode.get_keys_in_this_Node()) // record node
    {
