...
public class DBF implements Closeable, HasSize {
  ...
  /**
   * Returns matching Visual Foxpro codepage, or {@value CodePage#NO_CODEPAGE} if none match the
   * byte value.
   *
   * @return
   */
  ...
  /**
   * Sets the codepage byte value in the DBF header and also switches the GLOBAL DBF to the new
  ...
  /**
   * creates a new DBF file or replaces an existing database file, w/o format assumes dbaseiii file
   * format.
   *
   ...
   * @throws SecurityException Java error caused by called methods, most likely trying to create on
   *     a remote system
   */
   ...
   * will try 5 times within the fileLockTimeOut specified in org.xBaseJ.property fileLockTimeOut,
  ...
  /**
   * opens an Index file associated with the database. This index becomes the primary index used in
   * subsequent find methods.
   *
  ...
  /**
   * used to find a record with an equal or greater string value. when done the record pointer and
   * field contents will be changed.
   *
  ...
  /**
   * used to find a record with an equal and at the particular record. when done the record pointer
  ...
  /**
   * used to find a record with an equal string value. when done the record pointer and field
   * contents will be changed only if the exact key is found.
   *
   * @param keyString a search string
  ...
  /**
   * used to get the next record in the index list. when done the record pointer and field contents
   * will be changed.
   *
   * @param lock - boolean lock record indicator
   * @throws xBaseJException org.xBaseJ Index not opened or not part of the database eof - end of
   *     file
   * @throws IOException Java error caused by called methods
   */
  ...
  /**
   * used to get the previous record in the index list. when done the record pointer and field
   * contents will be changed.
   *
   * @param lock boolean lock record indicator
   * @throws xBaseJException org.xBaseJ Index not opened or not part of the database tof - top of
   *     file
   * @throws IOException Java error caused by called methods
   */
  ...
  /**
   * used to read the next record, after the current record pointer, in the database. when done the
  ...
  /**
   * used to read a record at a particular place in the database. when done the record pointer and
   * field contents will be changed.
   *
   * @param recno the relative position of the record to read
   * @param lock - boolean lock record indicator
   * @throws xBaseJException passed an negative number, 0 or value greater than the number of
   *     records in database
   * @throws IOException Java error caused by called methods
   */
  ...
  /**
   * used to position record pointer at the first record or index in the database. when done the
   * record pointer will be changed. NO RECORD IS READ. Your program should follow this with either
   * a read (for non-index reads) or findNext (for index processing)
   *
  ...
  /**
   * used to position record pointer at the last record or index in the database. when done the
   * record pointer will be changed. NO RECORD IS READ. Your program should follow this with either
   * a read (for non-index reads) or findPrev (for index processing)
   *
   ...
   * database.
   *
   * @param lock - boolean lock indicator - locks the entire file during the write process and then
   *     unlocks the file.
   ...
   *
   * @param lock - boolean lock indicator - locks the entire file during the write process and then
   *     unlocks the file. Also record lock will be released, regardless of parameter value
   * @throws xBaseJException any one of several errors
   * @throws IOException Java error caused by called methods
   */
  ...
  public void close() throws IOException {
    ...
    /**
     * reset file length if length doesn't match calculated length, this is required for the DBF to
     * be readable by Visual FoxPro 9.x
     */
    if (!readonly) {
      /*
   ...
   * @throws xBaseJException usually occurs when Field number is less than 1 or greater than the
   *     number of fields
   */
  ...
  public void getXML(PrintWriter pw) throws IOException, xBaseJException {
    ...
    int i;
    pw.println(
        "<dbf name=\"" + Util.normalize(getName()) + "\" encoding=\"" + getEncodingType() + "\">");
    Field fld;
