...
public class DbfViewer extends JFrame
  ...
  private boolean fileChangedTestDontContinue() {
    ...
    return JOptionPane.showConfirmDialog(
            null, "Data has changed. Ok to continue. Cancel to return.", "Confirm", dialogButton)
...
class dbfTableRecordMemoDialog extends JDialog implements ActionListener, WindowListener {
  ...
  public dbfTableRecordMemoDialog(
      DbfViewer inParent, JPanel inFr, dbfTableModel indbfTableModel, int inRow, int inCol) {
...
class dbfViewerSearchDialog extends JDialog implements ActionListener, WindowListener {
  ...
  public dbfViewerSearchDialog(
      DbfViewer inParent, dbfTableModel inModel, String inText, boolean inFields[]) {
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (event.getSource() == Okay) {
      ...
      if (s.length() < 1) {
        JOptionPane.showMessageDialog(
            parent, "Enter Search Text", "Enter Search Text", JOptionPane.ERROR_MESSAGE);
        return;
      }
      int i;
