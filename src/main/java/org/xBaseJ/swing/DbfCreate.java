  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    if (ae.getSource() == menuOpen) {
      ...
      try {
        ...
        if (dbf.getVersion() == DBFTypes.DBASEIV
            || dbf.getVersion() == DBFTypes.DBASEIV_WITH_MEMO) {
          menuType.setText("Type: dBaseIV");
          typeIV.setState(true);
          typeIII.setState(false);
          typeFP.setState(false);
        } else if (dbf.getVersion() == DBFTypes.DBASEIII
            || dbf.getVersion() == DBFTypes.DBASEIII_WITH_MEMO) {
        ...
        JOptionPane.showMessageDialog(
            this, xex.getMessage(), "xBaseJException Occurred", JOptionPane.ERROR_MESSAGE);
        repaint();
        return;
      } catch (IOException ioex) {
        JOptionPane.showMessageDialog(
            this, ioex.getMessage(), "IOException Occurred", JOptionPane.ERROR_MESSAGE);
        repaint();
        return;
      }
    ...
    if (ae.getSource() == menuCreate) {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            this, nfex.getMessage(), "NumberFormatException Occurred", JOptionPane.ERROR_MESSAGE);
        repaint();
        this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
        return;
      } catch (IOException ioex) {
       ...
       * ex.getMessage(), "Exception Occurred",
       * JOptionPane.ERROR_MESSAGE); repaint(); this.setCursor(new
       * Cursor(Cursor.DEFAULT_CURSOR)); return; }
       */
      this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
      setTitle("Create dBase File: " + fil.getName());
    }
...
class dbfCreateModel extends AbstractTableModel {
  ...
  int inRow;
  String columnName[] = {"Name", "Type", "Length", "Dec. Position", "Indexed", "Index Name"};
  int columnCount = 6;
  int rowCount;
  Boolean deleted[];
  JFrame parent;
